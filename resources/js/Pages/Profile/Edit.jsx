...
// Avatar Uploader
function AvatarUploader({ previewUrl, currentUrl, onFileChange, fileRef }) {
    ...
    return (
        <div className="flex flex-col items-center gap-4">
            <div
                ...
                onDrop={handleDrop}
            >
                <div className={`w-20 h-20 rounded-full overflow-hidden transition-all duration-300 ring-2 ring-offset-2 ring-offset-surface ${dragging ? 'ring-pink-dark scale-105' : 'ring-pink-dark'}`}>
                    <img
                        src={displaySrc}
                        alt="Profile"
            ...
            <input ref={fileRef} type="file" accept="image/jpeg,image/png,image/jpg" className="hidden" onChange={onFileChange} />

            <div className="text-center h-10 flex flex-col items-center justify-start">
                {previewUrl ? (
                    <span className="text-sm text-pink-dark font-semibold leading-tight">Not saved yet</span>
                ) : (
                    <p className="text-xs text-muted leading-tight">Click or drag to change photo</p>
                )}
            </div>
        </div>
    );
...
export default function ProfileEdit({ user }) {
    ...
    const inputCls = [
        ...
        'bg-fore font-sans outline-none transition-all duration-200 focus:border-pink-dark',
    ].join(' ');
    ...
    useEffect(() => {
        const handleBeforeUnload = (e) => {
            if (isDirty && !ignoreDirtyRef.current) { e.preventDefault(); e.returnValue = ''; }
        };
    ...
    const submit = (e) => {
        ...
        router.post('/profile', data, {
            ...
            onError: (errs) => {
                showToast('error', Object.values(errs)[0] || 'Something went wrong.');
            },
            onFinish: () => {
                setBusy(false);
    ...
    const confirmCancel = () => {
        ...
        cancelPreview();
        if (pendingVisit) {
            setTimeout(() => { router.visit(pendingVisit.url, pendingVisit); ignoreDirtyRef.current = false; }, 0);
        } else {
            ignoreDirtyRef.current = false;
        }
    };
        ...
        <AuthenticatedLayout>
                    ...
                    <div className="bg-surface rounded-2xl !p-4 shadow-[0_2px_20px_rgba(0,0,0,0.08)] overflow-hidden">
                        ...
                        <div className="!px-8 pb-0 flex items-center gap-6">
                            ...
                            <div className="flex flex-col justify-center">
                                ...
                                {form.position && (
                                    <span className="inline-block mt-2 px-3 py-0.5 rounded-full text-[11px] font-bold text-white w-max bg-pink-dark">
                                        {form.position}
                                    </span>
                                )}
                            </div>
                        ...
                        <div className="!p-8 w-full">
                            <p className="text-xs font-bold text-muted uppercase tracking-widest !mb-2">Account Information</p>
                            ...
                            <div className="flex items-center justify-end gap-3 !mt-6">
                                <button
                                    ...
                                    disabled={busy}
                                    className="flex items-center gap-2 !px-8 !py-2 rounded-xl text-sm font-bold border border-gray-200 text-ink bg-fore hover:bg-border transition-all cursor-pointer"
                                >
                                    Cancel
                                </button>
                                <button
                                    type="submit"
                                    disabled={busy}
                                    className="flex items-center gap-2 !px-8 !py-2 rounded-xl text-sm font-bold text-white border-none cursor-pointer transition-all duration-200 hover:bg-pink-dark/80 disabled:opacity-60 disabled:cursor-not-allowed bg-pink-dark"
                                >
                                    {busy ? (
                                        <><Loader2 size={14} strokeWidth={3} className="animate-spin" /> Saving…</>
                                    ) : (
            ...
            {showCancelModal && (
                <ConfirmOverlay
                    ...
                    message={<>Are you sure you want to discard<br />your unsaved changes?</>}
                    cancelLabel="Keep Editing"
                    confirmLabel="Discard"
                    confirmVariant="primary"
                    onCancel={() => { setShowCancelModal(false); setPendingVisit(null); }}
